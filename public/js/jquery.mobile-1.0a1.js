...
$.widget( "mobile.scrollview", $.mobile.widget, {
	...
	_create: function() {
        var direction = $( this.element ).data( "type" ) || "both";
		var dirclass = "";
		if (direction == "horizontal") dirclass = " ui-scrollview-hz";
		if (direction == "vertical") dirclass = " ui-scrollview-vc";

		var o = this.options;
		this.element.css('color', 'green');
		//create listview markup
		this.element
			.addClass( "ui-scrollview"+dirclass )
			.attr( "role", "scrollbox" );

		if ( o.inset ) {
			this.element
				.addClass( "ui-scrollview-inset" );
		}

		//manage touch
		//see http://cubiq.org/dropbox/iscroll/iscroll.js?v3.7.1
		...
		this.element.bind('touchstart', {from: this.element}, this._touchStart);
		this.element.bind('touchmove', this._touchMove);
		this.element.bind('touchend', this._touchEnd);
		this.element.bind('touchcancel', this._touchCancel);
	},

	_touchStart: function (event) {
		$scroll = event.data.from;
		$offsetx = event.originalEvent.changedTouches[0].pageX;
		$offsety = event.originalEvent.changedTouches[0].pageY;
		event.preventDefault();
	...
	_touchMove: function (event) {
		...
		var dir = $($scroll).data("type");
		if (dir == "both" || dir == "vertical") {
			$scroll.scrollTop($scroll.scrollTop()-(event.originalEvent.changedTouches[0].pageY-$offsety));
			$offsety = event.originalEvent.changedTouches[0].pageY;
		}

		if (dir == "both" || dir == "horizontal") {
			$scroll.scrollLeft($scroll.scrollLeft()-(event.originalEvent.changedTouches[0].pageX-$offsetx));
			$offsetx = event.originalEvent.changedTouches[0].pageX;
		}
...
$.mobile.scrollview.prototype.options.filter = false;

$( ":mobile-scrollview" ).live( "scrollviewcreate", function() {
	var scroll = $( this ),
		scrollview = scroll.data( "scrollview" );
	if ( !scrollview.options.filter ) {
		return;
	}
	//get options
